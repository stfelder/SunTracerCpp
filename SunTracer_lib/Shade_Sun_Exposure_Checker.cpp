...
bool Shade_Sun_Exposure_Checker::is_sun_shining_on_shade(double shade_azimuth,
    ...
    return is_sun_shining_on_shade(sun_altitude, sun_azimuth, shade_azimuth, shade_direction_tolerance_start_in_deg,
                                   shade_direction_tolerance_end_in_degrees);
}

bool Shade_Sun_Exposure_Checker::is_sun_shining_on_shade(double sun_altitude, double sun_azimuth, double shade_azimuth, double shade_direction_tolerance_start_in_deg, double shade_direction_tolerance_end_in_degrees){
    //Check if sun is shining on shade
...
Shade_Sun_Exposure_Checker::Shade_Sun_Exposure_Checker(double longitude, double latitude) : _longitude(longitude), _latitude(latitude) {

}

bool Shade_Sun_Exposure_Checker::is_now_sun_shining_on_shade(double shade_azimuth) {
    // Hole die aktuelle Zeit als time_point
    auto now = std::chrono::system_clock::now();
    ...
    double sun_azimuth = _sunTracer.calculate_sun_azimuth_in_degrees(_latitude, _longitude,
                                                                     year_now, month_now, day_now, hour_now, minute_now, second_now);
    double sun_altitude = _sunTracer.calculate_sun_altitude_in_degrees(_latitude, _longitude,
                                                                      year_now, month_now, day_now, hour_now, minute_now, second_now);
    bool is_sun_shining = is_sun_shining_on_shade(sun_altitude, sun_azimuth, shade_azimuth, 5.0, 5.0);

    return false; //Todo write unittest
}
